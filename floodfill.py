...
def main():
    ...
    orient = 0   # north
    do_floodfill() # initial floodfill with no walls
    ...
    while True:
        ...
        if cells[current_loc[1]][current_loc[0]] == 0:
            # mouse is at goal
            log('done')
            read_write_matrix.write(cells, 'cells.txt')
            read_write_matrix.write(cell_type, 'cell_types.txt')
            break

        next_loc, direction = next_cell(current_loc)
